...
/******/ 	var hotCurrentHash = "5f80685eaa48e9f47ad1"; // eslint-disable-line no-unused-vars
...
var _voronoi = __webpack_require__(1);
...
var _poissonDiscSampler = __webpack_require__(3);

var _poissonDiscSampler2 = _interopRequireDefault(_poissonDiscSampler);

var _victor = __webpack_require__(4);
...
var voronoi = function voronoi() {
  ...
  var svg = d3.select('.d3').append('svg').attr('width', width).attr('height', height).on('touchmove mousemove', moved);

  function getStaticPoints() {
    var points = [];
    var sampler = (0, _poissonDiscSampler2.default)(width, height, width / 20);
    var sample;

    while (sample = sampler()) {
      points.push({
        position: _victor2.default.fromArray(sample),
        velocity: new _victor2.default(),
        cull: false
      });
    }
    return points;
  ...
  function getAnimatedPoints() {
    ...
    for (var i = 0; i < numPoints; i++) {
      var spawn = new _victor2.default(width / 2, height / 2);
      var radius = Math.random() * (width / 2);
      var angle = Math.random() * Math.PI * 2;
      var x = Math.cos(angle) * radius;
      var y = Math.sin(angle) * radius;
      var position = spawn.clone().add(new _victor2.default(x, y));
      var velocity = position.clone().subtract(spawn).normalize().multiply(new _victor2.default(0.15, 0.15));
      points.push({
        position: position,
        velocity: velocity,
        cull: false
  ...
  var voronoi = d3.voronoi().extent([[-1, -1], [width + 1, height + 1]]).x(function x(d) {
    return d.position.x;
  }).y(function y(d) {
    return d.position.y;
  });
  ...
  var spawnInterval = 1;
  var mouseColorScale = d3.scaleLinear().domain([0, 0.05, 0.2, 0.3, 0.5, 1]).range([d3.hsl('#FFFFEE'), d3.hsl('#FFFF00'), d3.hsl('#990000'), d3.hsl('#220022'), d3.hsl('#333333'), d3.hsl('#666666')]).interpolate(d3.interpolateHsl);
  function moved() {
    var currentTime = new Date();
    if (currentTime - lastTime > spawnInterval) {
      var position = _victor2.default.fromArray(d3.mouse(this));
      position.x += Math.random() * 10 - 5;
      sites.push({
        position: position,
        velocity: new _victor2.default(Math.random() * 2 - 1, -5 - Math.random() * 2),
        cull: true,
        start: currentTime,
  ...
  function update() {
    ...
    while (i--) {
      ...
      if (site.position.x <= 0 || site.position.x >= width || site.position.y <= 0 || site.position.y >= height) {
        if (site.cull) {
          sites.splice(i, 1);
        } else {
          var spawn = new _victor2.default(width / 2, height / 2);
  ...
  function color(d) {
    ...
    var mx = width / 2,
        ...
        colorScale = d3.scaleLinear().domain([1, 100, length * 0.9, length]).range([d3.hsl('#FFFFEE'), d3.hsl('#FFFF00'), d3.hsl('#330000'), d3.hsl('#110000')]).interpolate(d3.interpolateHsl);
